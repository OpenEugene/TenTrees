...
namespace OpenEug.TenTrees.Module.Enrollment.Services
{
    public class ServerEnrollmentService : IEnrollmentService
    {
        ...
        public Task<List<EnrollmentViewModel>> GetEnrollmentViewModelsAsync(int moduleId)
        {
            if (_userPermissions.IsAuthorized(_accessor.HttpContext.User, _alias.SiteId, EntityNames.Module, moduleId, PermissionNames.View))
            {
                return Task.FromResult(_enrollmentRepository.GetEnrollmentViewModels(moduleId).ToList());
            }
            else
            {
