...
Users with edit permissions organise growers and mentors into named, village-scoped cohorts so the program can track phases, run cohort-scoped reports, and manage each group independently. Translated from the retired feature file `Specs/Features/CohortManagement.feature` (high priority).

## Cohort lifecycle

A cohort belongs to a village and a year, and moves through three statuses:

- **Planned** — the status of every newly created cohort. On creation the system suggests a name of the form "Village Year" (e.g. "Roebuck 2026"), which can be accepted or overwritten with a custom name.
- **Active** — set by an authorized user; the cohort then appears in active cohort filters.
- **Completed** — the cohort leaves active filters by default but is visible when "Show completed cohorts" is enabled. **A completed cohort cannot be reactivated**; attempts to set it back to Active or Planned are rejected with "A completed cohort cannot be reactivated".
...
- At enrollment, cohort selection is **optional**. If an active cohort exists for the village the enroller may pick one; growers enrolled without a cohort can be added later from the grower status screen. See [Grower Enrollment](/Specifications/Grower-Enrollment).
- A grower can belong to **multiple cohorts** and appears when filtering by any of them.
...
- Admins can add a cohort via an "Add cohort" dropdown (which excludes Completed cohorts) and remove one via the badge's remove button with an inline "Remove …? Yes / No" confirmation.
- Mentors see the badges read-only — no remove buttons, no add dropdown.

## Class association

Training classes and cohorts can be linked from either side — the training edit screen ("Add cohort" dropdown) or the cohort edit screen ("Add class" dropdown). Links display as tags/badges on both screens and on the training index's cohorts column. Removing a link always asks an inline Yes/No confirmation; choosing "No" keeps the link. See also [Class Attendance](/Specifications/Class-Attendance).
...
- Cohort management lists all cohorts with status and household counts.
...
The summary for a cohort shows its member count, status, enrollment date range, and assigned tree mentors.

## Assessment frequency

Assessment frequency is based on the **cohort's** age, not the grower's oldest cohort. For example, a cohort activated in 2026 assesses twice monthly, while growers in a completed 2023 cohort retain a monthly frequency. Each cohort's configurable frequency drives the assessment date proximity warning — see [Garden Assessment](/Specifications/Garden-Assessment).

## Reporting

Program reports can be scoped to a specific cohort so results reflect only that cohort's households, including visit-count summaries for funder reporting (e.g. 25 home visits for a cohort in a month). See [Program Reporting](/Specifications/Program-Reporting).

## Scenarios (Gherkin)

The original scenarios, preserved verbatim from the retired `Specs/Features/CohortManagement.feature` as the precise acceptance criteria for the behaviour described above.
